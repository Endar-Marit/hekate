...
int create_config_entry()
{
	...
	{
		u8 res = f_open(&fp, "bootloader2/hekate_ipl.ini", FA_READ);
		if (res == FR_NO_FILE || res == FR_NO_PATH)
		{
			f_mkdir("bootloader2");
			f_mkdir("bootloader2/ini");
			f_mkdir("bootloader2/payloads");
			f_mkdir("bootloader2/sys");
		}
		else
		{
			if (!res)
	...
	if (f_open(&fp, "bootloader2/hekate_ipl.ini", FA_WRITE | FA_CREATE_ALWAYS) != FR_OK)
		return 1;
	// Add config entry.
	f_puts("[config]\nautoboot=", &fp);
...
static void _config_autoboot_list(void *ent)
{
	...
	if (sd_mount())
	{
		if (ini_parse(&ini_sections, "bootloader2/ini", true))
		{
